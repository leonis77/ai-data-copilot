    ...
    <div className="min-h-screen">
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        {floatingElements.map((el, i) => (
          <motion.div key={i} className="absolute hidden lg:block" style={{ left: el.x, top: el.y }}
            animate={{ y: [0, -15, 0] }} transition={{ duration: 4, delay: el.delay, repeat: Infinity, ease: "easeInOut" }}>
            <div className="glass p-4 rounded-2xl"><el.icon className="w-6 h-6 text-primary-light/50" /></div>
          </motion.div>
        ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            ...
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
              ...
              {i < 2 && <div className="hidden md:block absolute top-1/2 -right-4 w-8 h-0.5 bg-gradient-to-r from-primary/50 to-accent-purple/50" />}
            </motion.div>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 py-24">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          style={{ background: "linear-gradient(135deg, rgba(99,102,241,0.15), rgba(168,85,247,0.1), rgba(6,182,212,0.08))" }}>
      ...
      <footer className="border-t border-white/5 py-8">
        <div className="max-w-7xl mx-auto px-6 text-center text-sm text-white/30">
          Commerce Copilot &copy; 2025. Powered by DeepSeek AI.
        </div>
      </footer>
    </div>
  );
