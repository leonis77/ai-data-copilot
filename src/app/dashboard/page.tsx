...
var numStats = stats ? Object.entries(stats.stats) : [];
  ...
  var diagnosis: any[] = [];
  var healthScore: any = null;
  if (datasetData?.rows?.length > 0) {
    try {
      var rows = datasetData.rows || [];
      var cols = datasetData.columns || [];
      var nameCol = findCol(cols, [/名称/,/商品/,/产品/,/name/,/title/]);
      var priceCol = findCol(cols, [/金额/,/价格/,/price/,/amount/,/实付/]);
      var qtyCol = findCol(cols, [/数量/,/quantity/,/qty/]);
      var stockCol = findCol(cols, [/库存/,/stock/]);
      if (nameCol && priceCol) {
  ...
  var criticalIssues = diagnosis.filter(function(d) { return d.level === "critical"; });
  var warnings = diagnosis.filter(function(d) { return d.level === "warning"; });
  var opportunities = diagnosis.filter(function(d) { return d.level === "opportunity"; });
  var hasDiagnosis = diagnosis.length > 0;
    ...
    <div className="min-h-screen py-12"><div className="max-w-7xl mx-auto px-6">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <div className="flex items-center gap-3 mb-2"><div className="w-10 h-10 rounded-xl bg-primary/20 flex items-center justify-center"><BarChart3 className="w-5 h-5 text-primary-light" /></div>
          <div><h1 className="text-3xl font-bold"><span className="gradient-text">经营诊断</span></h1></div>
        ...
        <div className="space-y-6">
          ...
          {criticalIssues.length > 0 && (
            <div>
              ...
              <div className="space-y-2">{criticalIssues.map(function(d, i) { return <RiskCard key={i} title={d.title} detail={d.detail} level="critical" />; })}</div>
            </div>
          )}
          ...
          {warnings.length > 0 && (
            <div>
              ...
              <div className="space-y-2">{warnings.map(function(d, i) { return <RiskCard key={i} title={d.title} detail={d.detail} level="warning" />; })}</div>
            </div>
          )}
          ...
          {opportunities.length > 0 && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">{opportunities.map(function(d, i) { return <OpportunityCard key={i} title={d.title} detail={d.detail} action={d.action} impact={d.impact} />; })}</div>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-4 border-t border-white/5">
            ...
            {d1 && <GlassCard><BarChart title={distCols[1]} data={Object.entries(d1).slice(0,8).map(function(e) { return { name: e[0], value: e[1] }; })} /></GlassCard>}
          </div>
        </div>
      ) : (
        <div className="space-y-6">
