...
export default function UploadPage() {
  ...
  var { getRootProps, getInputProps, isDragActive } = useDropzone({ onDrop, accept: {
  ...
  var handleUpload = async function() {
    ...
    try {
      ...
      if (!res.ok) { var err = await res.json().catch(function() { return {}; }); throw new Error(err.error || "\u89e3\u6790\u5931\u8d25"); }
  ...
  return (
    <div className="min-h-screen py-12 pt-20"><div className="max-w-4xl mx-auto px-6">
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.6}} className="text-center mb-12 relative">
        ...
        <h1 className="text-4xl md:text-5xl font-bold mb-4"><span className="gradient-text">{"\u4e0a\u4f20\u6570\u636e"}</span><span className="text-white/90">{"\uff0cAI \u81ea\u52a8\u89e3\u6790"}</span></h1>
      ...
      {!result ? (
        <motion.div key="upload"><GlassCard gradient className="p-10">
          <div {...getRootProps()} className={"relative border-2 border-dashed rounded-2xl p-16 text-center cursor-pointer transition-all " + (isDragActive ? "border-indigo-400 bg-indigo-500/5" : "border-white/10 hover:border-white/20 hover:bg-white/[0.02]")}>
            ...
            {file ? (
              ...
              <div className="space-y-4">
                ...
                <div><p className="font-semibold text-lg text-white/60">{isDragActive ? "\u677e\u5f00\u4e0a\u4f20" : "\u62d6\u62fd\u6587\u4ef6\u5230\u6b64\u5904\u6216\u70b9\u51fb\u9009\u62e9"}</p><p className="text-sm text-white/30 mt-1">.xlsx .xls .csv{"\uff0cAI \u81ea\u52a8\u8bc6\u522b\u8868\u683c\u7ed3\u6784"}</p><button onClick={function(e) { e.stopPropagation(); downloadTemplate(); }} className="mt-4 text-xs text-indigo-400/70 hover:text-indigo-400 transition-colors underline">{"\u4e0b\u8f7d\u6807\u51c6\u6a21\u677f (CSV)"}</button></div>
              </div>
            )}
          </div>
          ...
          {file && <motion.div initial={{opacity:0}} animate={{opacity:1}} className="mt-6 flex justify-center">
            <button onClick={handleUpload} disabled={uploading} className="group flex items-center gap-2 px-8 py-3 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-semibold disabled:opacity-50 transition-all hover:shadow-lg hover:shadow-indigo-500/25">
              {uploading ? <><div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />AI {"\u89e3\u6790\u4e2d..."}</> : <><Sparkles className="w-5 h-5" />{"\u5f00\u59cb\u89e3\u6790"}</>}
            </button>
          </motion.div>}
