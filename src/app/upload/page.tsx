...
export default function UploadPage() {
  ...
  var fileDataRef = useRef("");  // ref avoids closure issues

  var clearAll = function() {
    localStorage.removeItem("aicopilot");
    setFile(null); setResult(null); setCols([]); setSheets([]);
    setSelectedSheet(""); setTemplate(null); fileDataRef.current = "";
  };
  ...
  var onDrop = useCallback(function(files: File[]) {
    ...
    if (ext !== "xlsx" && ext !== "xls" && ext !== "csv") {
      setError("\u4ec5\u652f\u6301 xlsx / xls / csv \u683c\u5f0f"); return;
    }
  ...
  var handleUpload = async function(sheet?: string) {
    ...
    try {
      // Always get fresh base64 for the current file
      var b64 = await fileToBase64(file);
      // First call: store in ref; subsequent calls: use stored ref value  
      if (!sheet) fileDataRef.current = b64;
      var body: any = { fileName: file.name, fileData: sheet ? fileDataRef.current : b64 };
      ...
      if (data.sheets && data.sheets.length > 1 && !sheet) {
        setSheets(data.sheets); setSelectedSheet(data.sheets[0].name); return;
      }
        ...
        <motion.div key="sheets" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.5}} className="space-y-6">
          <GlassCard gradient>
            ...
            <div className="mt-6 flex justify-center gap-4">
              <button onClick={function() { setFile(null); setSheets([]); setSelectedSheet(""); fileDataRef.current = ""; }} className="px-6 py-3 rounded-xl glass text-white/60 hover:text-white transition-colors font-medium">{"\u53d6\u6d88"}</button>
