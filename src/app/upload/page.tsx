...
export default function UploadPage() {
  ...
  return (
    <div className="min-h-screen py-12"><div className="max-w-4xl mx-auto px-6">
      ...
      {!result ? (
        <motion.div key="upload" exit={{ opacity: 0, y: -20 }}><GlassCard gradient className="p-10">
        ...
        <motion.div key="result" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
          <GlassCard gradient>
            <div className="flex items-center gap-3 mb-6"><div className="w-10 h-10 rounded-xl bg-green-500/20 flex items-center justify-center"><CheckCircle className="w-5 h-5 text-green-400" /></div><div><h3 className="font-semibold text-lg">解析成功</h3><p className="text-sm text-white/40">已识别数据结构</p></div></div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[{ label: "行数", value: result.rowCount }, { label: "字段", value: result.columns.length }].map(function(s, i) {
                return <div key={i} className="text-center p-4 rounded-xl bg-white/[0.03]"><CountUp end={s.value} className="text-2xl font-bold gradient-text block mb-1" /><p className="text-xs text-white/40">{s.label}</p></div>;
              })}
            </div>
            <div className="mt-6 p-4 rounded-xl bg-white/[0.03]">
              <h4 className="text-sm font-medium text-white/60 mb-2">字段列表</h4>
              <div className="flex flex-wrap gap-2">{result.columns.map(function(col: string) { return <span key={col} className="px-3 py-1.5 rounded-lg bg-primary/10 text-primary-light text-xs font-medium">{col}</span>; })}</div>
            </div>
            <div className="mt-6 p-4 rounded-xl bg-white/[0.03]"><h4 className="text-sm font-medium text-white/60 mb-2">数据摘要</h4><p className="text-sm text-white/50 leading-relaxed">{result.summary}</p></div>
          </GlassCard>
          <div className="flex justify-center gap-4">
            <button onClick={function() { setFile(null); setResult(null); }} className="px-6 py-3 rounded-xl glass text-white/60 hover:text-white transition-colors font-medium">重新上传</button>
            <button onClick={function() { router.push("/dashboard"); }} className="group flex items-center gap-2 px-8 py-3 rounded-xl bg-gradient-to-r from-primary to-accent-purple text-white font-semibold hover:shadow-lg hover:shadow-primary/25 transition-all">查看仪表盘<ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" /></button>
          </div>
        </motion.div>
      )}
    </div></div>
