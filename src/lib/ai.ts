...
function buildSystemPrompt(tableClass: TableClass): string {
  ...
  switch (tableClass) {
    case "order":
      base += "\n当前数据是**订单表**。重点关注：GMV、客单价、复购率、商品排行和集中度、时间趋势、地域分布、定价策略。\n";
      break;
    case "supply":
      base += "\n当前数据是**供货表**。重点关注：供应商集中度、采购价格合理性、供需匹配、最小起订量、产地标准化。\n";
      break;
    case "inventory":
      base += "\n当前数据是**库存表**。重点关注：库存周转率、库龄结构、安全库存预警、仓库分布合理性。\n";
      break;
    case "financial":
      base += "\n当前数据是**财务流水**。重点关注：收支结构趋势、异常交易、费用分类占比。\n";
      break;
    default:
