  ...
  return (
    <span>
      ...
      {!done && <motion.span animate={{ opacity: [1, 0] }} transition={{ duration: 0.6, repeat: Infinity, repeatType: "reverse" }}
        className="inline-block w-1.5 h-4 bg-blue-400 ml-0.5 rounded-sm align-middle" />}
    </span>
  );
}
...
export function AIInsightPanel({ analysis, loading, onRegenerate }: AIInsightPanelProps) {
  ...
  if (loading) {
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
        className="relative overflow-hidden rounded-2xl p-8 border border-blue-500/10"
        style={{ backdropFilter: "blur(20px)", background: "radial-gradient(circle at 50% 50%, rgba(59,130,246,0.08), transparent 60%), rgba(243,244,246,0.5)" }}>
        <div className="flex flex-col items-center gap-6">
          <motion.div animate={{ rotate: 360 }} transition={{ duration: 3, repeat: Infinity, ease: "linear" }}
            className="w-14 h-14 rounded-full border-2 border-blue-400/20 border-t-blue-400" />
          <p className="text-sm text-blue-400/80 font-medium">[ AI Thinking... ]</p>
          <div className="space-y-2 w-full max-w-xs">
            {steps.map((step, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={i < thinkingStep ? { opacity: 1, x: 0 } : { opacity: 0.2 }}
                ...
                <CheckCircle className={`w-4 h-4 ${i < thinkingStep ? "text-emerald-400" : "text-faint"}`} />
                <span className={`text-sm ${i < thinkingStep ? "text-secondary" : "text-faint"}`}>{step}</span>
              </motion.div>
            ))}
          </div>
        </div>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="space-y-4">
      {analysis.summary && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          className="relative overflow-hidden rounded-2xl p-6 border border-blue-500/10"
          style={{ backdropFilter: "blur(20px)", background: "radial-gradient(ellipse at 0% 0%, rgba(59,130,246,0.1), transparent 50%), rgba(243,244,246,0.5)" }}>
          <div className="flex items-start gap-3">
            <div className="w-8 h-8 rounded-lg bg-blue-500/10 flex items-center justify-center shrink-0">
              <Brain className="w-4 h-4 text-blue-400" />
            </div>
            <div>
              <p className="text-xs text-blue-400/60 uppercase tracking-wider mb-2">AI {"\u6838\u5fc3\u6d1e\u5bdf"}</p>
              <p className="text-sm text-primary leading-relaxed"><TypewriterText text={analysis.summary} speed={25} /></p>
            </div>
          </div>
        </motion.div>
      )}
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          ...
          {analysis.insights.map((insight: string, i: number) => (
            <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
              transition={{ delay: 0.3 + i * 0.1 }} whileHover={{ scale: 1.02, borderColor: "#3B82F6", transition: { duration: 0.15 } }}
              className="flex items-start gap-3 p-4 rounded-xl border border-gray-100"
              style={{ backdropFilter: "blur(12px)", background: "rgba(243,244,246,0.4)" }}>
      ...
      {analysis.risks && analysis.risks.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="space-y-2">
          <p className="text-xs text-faint uppercase tracking-wider flex items-center gap-2">
